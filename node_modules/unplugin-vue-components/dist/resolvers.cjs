...
function getStyleDir(compName) {
  ...
  if (!styleDir)
    styleDir = _chunk5RDO6MHScjs.kebabCase.call(void 0, compName);
  return styleDir;
}
...
function resolveComponent(name, options) {
  ...
  const partialName = _chunk5RDO6MHScjs.kebabCase.call(void 0, name.slice(2));
...
function ElementPlusResolver(options = {}) {
  ...
  async function resolveOptions() {
    ...
    optionsResolved = {
      ssr: false,
      version: await _chunk5RDO6MHScjs.getPkgVersion.call(void 0, "element-plus", "2.2.2"),
      importStyle: "css",
      directives: true,
      exclude: void 0,
  ...
  return {
    ...
    resolve: (name) => {
      ...
      if (/^El[A-Z]/.test(name)) {
        ...
        const partialName = _chunk5RDO6MHScjs.kebabCase.call(void 0, compName);
...
function IduxResolver(options = {}) {
  return {
    ...
    resolve: async (name) => {
      ...
      if (!packageName)
        return;
      const resolvedVersion = await _chunk5RDO6MHScjs.getPkgVersion.call(void 0, `${scope}/${packageName}`, "2.0.0");
      let dirname = specialComponents[name];
      if (!dirname) {
        const nameIndex = packageName === "pro" ? 2 : 1;
        dirname = _chunk5RDO6MHScjs.kebabCase.call(void 0, name).split("-")[nameIndex];
      }
...
var moduleType = _chunk5RDO6MHScjs.isSSR ? "lib" : "es";
function getSideEffects5(dirName, options) {
  const { importStyle = true } = options;
  if (!importStyle || _chunk5RDO6MHScjs.isSSR)
    return;
  if (importStyle === "less")
    ...
    resolve: (name) => {
      if (name.startsWith("Van")) {
        ...
        return {
          ...
          sideEffects: getSideEffects5(_chunk5RDO6MHScjs.kebabCase.call(void 0, partialName), options)
        };
      }
    }
  };
...
function getResolved(name, options) {
  ...
  if (importStyle || importCss) {
    if (importStyle === "less" || importLess)
      sideEffects.push(`${path}/es/${_chunk5RDO6MHScjs.kebabCase.call(void 0, name)}/style/less`);
    else
      sideEffects.push(`${path}/es/${_chunk5RDO6MHScjs.kebabCase.call(void 0, name)}/style/index${styleExtname}`);
  }
  return {
    from: path,
...
var formatters = {
  "kebab-case": _chunk5RDO6MHScjs.kebabCase,
  "camelCase": _chunk5RDO6MHScjs.camelCase,
  "PascalCase": _chunk5RDO6MHScjs.pascalCase
};
...
function getCompDir(compName) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (compName.match(matcher.pattern)) {
      compPath = `${matcher.compDir}/${_chunk5RDO6MHScjs.kebabCase.call(void 0, compName)}.vue`;
      break;
    }
  }
  if (!compPath)
    compPath = _chunk5RDO6MHScjs.kebabCase.call(void 0, compName);
  return compPath;
}
...
function componentsResolver(name, { ssr }) {
  if (!name.match(/^D[A-Z]/))
    return;
  const resolveId = _chunk5RDO6MHScjs.kebabCase.call(void 0, name = name.slice(1));
  return {
    name,
...
function directivesResolver(name, { ssr }) {
  const resolveId = _chunk5RDO6MHScjs.kebabCase.call(void 0, name);
  return {
    name: `${name}Directive`,
    sideEffects: getSideEffects8(resolveId, "style.css"),
...
function getComponentStyleDir(importName, importStyle) {
  if (["ConfigProvider", "Icon"].includes(importName))
    return void 0;
  let componentDir = _chunk5RDO6MHScjs.kebabCase.call(void 0, importName);
  ...
  return {
    ...
    resolve: (name) => {
      if (canResolveIcons(options.resolveIcons)) {
        const iconPrefix = _chunk5RDO6MHScjs.pascalCase.call(void 0, getResolveIconPrefix(options.resolveIcons));
